...
Defines how the event page behaves when a `lock` query param targets a specific showtime variant, ensuring all sibling variants of the locked showtime are displayed, the originally-linked variant stays selected, unknown variants fall back gracefully, and the "view other showtimes" affordance reflects the locked showtime group.
